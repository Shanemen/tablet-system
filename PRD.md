...
**重要決策：打印文件僅需黑白，預覽保留顏色**

- **信徒預覽（線上查看）**：
  - 顯示彩色牌位圖片（黃色或紅色背景）
  - 使用 `@vercel/og` (基於 Satori) 動態生成彩色預覽圖
  - 幫助信徒確認牌位內容正確性
  
- **批量導出PDF（供管理員打印）**：
  - **PDF文件僅生成黑白版本**
  - 原因：管理員使用物理彩色紙張（黃紙或紅紙）進行打印
  - PDF中的黑白內容打印在彩色紙張上，最終效果與彩色PDF相同
  - 節省PDF文件大小，加快生成速度
  - 實現方式：批量導出時將彩色圖片轉換為灰度圖像後再組裝PDF

- **打印流程**：
  1. 管理員批量導出6個PDF文件（按牌位類型分組）
  2. 每個PDF文件為黑白版本
  3. 管理員根據PDF文件名選擇對應的彩色紙張：
     - `長生祿位.pdf` → 使用紅紙打印
     - 其他5種 → 使用黃紙打印
  4. 打印機輸出黑白內容到彩色紙張，最終呈現正確的視覺效果
