...
## 背景知识：姓名长度分布 (Background Knowledge: Name Length Distribution)

了解用户姓名的长度分布对于设计稳健的 UI 和生成算法至关重要。以下数据基于统计研究（2024）。

### 🇺🇸 英语姓名 (English Names - US Users)

- **典型值 (50%)**: 全名长度在 **11–15 字符**之间（First + Last，不含中间名）。
- **常见长名 (30%)**: 全名超过 **15 字符**（16–20 字符）。
- **长名 (10%)**: 全名超过 **20 字符**（21–30 字符，UI 容易爆掉的临界点）。
- **超长名 (1–2%)**: 全名超过 **30 字符**（有连字符/双姓时，必须有自动缩放或折行策略）。
- **短名 (5–8%)**: 全名短于 **10 字符**。

**结论**：
- 约 **40%** 的美国用户姓名长于"典型值"（>15 字符）。
- 约 **10%** 的用户姓名长到足以破坏普通 UI（>20 字符）。
- 必须支持 **多行显示** 和 **动态字体缩放**。

### 🇨🇳 中文姓名 (Chinese Names)

- **绝大多数 (98%)**: 2-3 个汉字（如：王明、李小花）。
- **少数 (1%)**: 4-5 个汉字（复姓或少数民族姓名）。
- **极少数**: 6+ 个汉字（通常是法名或特殊称谓）。

**结论**：
- 中文姓名长度相对固定，**垂直排版**是主要挑战。
- 默认字号应适配 3-4 字，超长名字需缩小字号。
