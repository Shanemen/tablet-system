...
1. **長生祿位 — ✅ 已完成并验证**（`prototypes/atlanta/longevity.html`）。已跑通：固定字放大、
   `闔家`单独成列、长名自动缩小、英文/越南文横排堆行、名字 ≤ 固定字、名字不压两侧卍纹。
   **已定下来的参数（实现阶段直接用）：**
   - 中间竖带：x **80–240**（收窄到两侧卍纹净空内，居中 x=160）、y **195–716**
   - 固定字 **38px**；名字默认 38px，**代码硬性约束 ≤ 固定字**（永不超过 `佛光注照`）
   - 横排 Latin：按宽度缩放 + 8px 内边距，确保不碰卍纹；家属后缀统一渲染成 `&family`（比 `and family` 短 → 字体更大），合并一行
   - 装饰边框：`prototypes/atlanta/frame-blank.svg`（道场提供的无文字版）
2. 往生蓮位：称谓(`祖父`)单列 + 长名分列 + 左侧 `陽上/稱謂/敬薦` 标签（下一个）
3. 祖先：1字 / `愛新覺羅` 2×2 + 固定字缩小
