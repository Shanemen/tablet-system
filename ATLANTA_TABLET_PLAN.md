...
| 嬰靈 aborted-spirits | 佛力超薦 | 往生蓮位 | 陽上 / 父 / 母 / 敬薦 | 固定字 **36**；描述每列填满 6 字再溢出；左区父+母双亲 |
...
- 验证点：中区三段 `佛力超薦` / 描述[**动态**] / `往生蓮位`（固定字 **36px**）；中文描述**每列填满 6 字再溢出到下一列**（`陳氏流產嬰孩`/`陳洛山`、`王氏二位嬰靈`/`菩薩`），右先读；英文/越南文横排；左区是**双亲** `陽上 / 父[名] / 母[名] / 敬薦`（中文竖排堆叠；英/越 `Father…/Mother…`、`Cha…/Mẹ…` 两条旋转行上下错开不重叠）
- 定稿参数：固定字 **36**；每列上限 6 字（填满即溢出）；左区同往生蓮位（底标签 `敬薦` 对齐 716、居中）
